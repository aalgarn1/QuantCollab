class Analyzer{
	constructor(player1, player2, lines){
		...
		this.interactiondynamicDATA = new Array();
    ...
    {
    	this.interactiondynamicDATA.push(data);
    }
	...
	calculateLinesPerIdea(){
		...
		for(var i = 0; i<this.interactionStorage.length; i++){
			if(this.interactionStorage[i].turns[0].inactivePlayer == this.player1){
				p1Lines += this.interactionStorage[i].getNumLines(); 
				p1IdeaCount++;
			} else if(this.interactionStorage[i].turns[0].inactivePlayer == this.player2){
	...
	calculateLinesAddedToNewIdeas(){
		...
		for(var i = 0; i<this.interactionStorage.length; i++){
			for(var j = 0; j<this.interactionStorage[i].turns.length; j++){
				if(this.interactionStorage[i].turns[0].inactivePlayer == this.player1){
					if(this.interactionStorage[i].turns[j].activePlayer == this.player2){
...
/*
	...
	calculateCoupledStartCount(){
		...
		for(var i = 0; i<this.interactionStorage.length; i++){
			...
			if(this.interactionStorage[i].getFoundedBy() == this.player1){
...
*/
	...
	getInteractionDataRecord(){
		var record = '';
		for (var i = 0; i < this.interactiondynamicDATA.length; i++){
			var curRecord1 = this.interactiondynamicDATA[i].activePlayer + ',';
			var curRecord2 = this.interactiondynamicDATA[i].objectLabel + ',';
			var curRecord3 = this.interactiondynamicDATA[i].interactionDYNAMIC + ',';
			record = record.concat(curRecord1 +curRecord2 + curRecord3); 
		}
		return record;
	...
	calculateSimilarInteractions(){
		...
		for(var i = 0; i<this.turns.length; i++){
			...
			if(this.turns[i].activePlayer == this.player1 && this.turns[i].similarity =="similar"){
				p1Sim++; 
			}else if (this.turns[i].activePlayer == this.player2 && this.turns[i].similarity =="similar"){
				p2Sim++; 
			}
		}
